class HKalkulator {
    ...
    _render() {
        ...
        ["Schritt", "Op.", "Prozentsatz [%]", "Betrag [€]"].forEach((text) => {
                        ...
                        if (base !== null) {
                            ...
                            if (absEl2 && absEl2.tagName === "INPUT") {
                                ...
                                if (
                                    ...
                                    computed = this._round(
                                        (base * (pct / 100)) / (1 - pct / 100),
                                    );
                                } else {
                                    computed = this._round((base * pct) / 100);
                                }
                                absEl2.value = computed.toFixed(2);
                            }
                        }
                    }
                    this._recalc();
                        ...
                        if (base !== null && base !== 0) {
                            ...
                            if (pctEl2) {
                                ...
                                if (
                                    ...
                                    pctComputed = this._round(
                                        (100 * abs) / (base + abs),
                                    );
                                } else {
                                    pctComputed = this._round8(
                                        (100 * abs) / base,
                                    );
                                }
                                pctEl2.value = pctComputed.toFixed(2);
                            }
                        }
                    }
                    this._recalc();
    ...
    _recalc() {
        let running = this._round(this._val(this.rows[0].id, "abs"));
        ...
        for (let i = 1; i < this.rows.length; i++) {
            ...
            let abs = this._val(row.id, "abs");

            if (row.special === "skonto" || row.special === "rabatt") {
                if (pct > 0) {
                    abs = this._round(
                        (running * (pct / 100)) / (1 - pct / 100),
                    );
                ...
                if (pct > 0) {
                    abs = this._round(running * (pct / 100));
                    this._set(row.id, "abs", abs);
                }
            } else if (
            ...
            if (row.op === "-") running = this._round(running - abs);
            else if (row.op === "+") running = this._round(running + abs);
        }
    }
